...
import { createResponse, CustomError, StatusCodes, toError } from 'utils'
...
export const handle: APIGatewayProxyHandler = ({ body }: APIGatewayEvent) => {
  ...
  const toResponse = (result: E.Either<CustomError, number>) => pipe(
    result,
    E.fold(
      ({ message }: Error) => createResponse(StatusCodes.BadRequest)({ message }),
      (ok: number) => createResponse(StatusCodes.Created)({ ok }),
    )
  )

  return pipe(
    TE.chain(UserRepository.save)(validateBody(body)),
    TE.map(toResponse),
